...
import { Message, Publication } from "../database/models";
...
export const getFromUser = (user_id: string) => {
    return Publication.findAll({ where: { user_id }, include: [{ model: Message, as: "messages" }] }).then((publications: PublicationModel[]) => {
